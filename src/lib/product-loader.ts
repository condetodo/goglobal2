...
export function parseProductRoadmap(md: string): ProductRoadmap | null {
  ...
  try {
    ...
    // Pattern: ### number. title followed by description until next ### number. or end
    // Handle both Windows (\r\n) and Unix (\n) line endings
    // The lookahead only looks for the next ### number. section, not ## headings
    const sectionMatches = [...md.matchAll(/###\s+(\d+)\.\s*(.+?)\r?\n([\s\S]*?)(?=\r?\n+###\s+\d+\.|$)/g)]
